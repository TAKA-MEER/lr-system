...
- **実際の工場環境（雑音・複数人の同時発言）はまだ検証していない**：これまでの検証はクリアな合成音声のみで行っています。
...
- **長時間試験でのコンテキストウィンドウ制約は解消済み**：3時間規模の耐久試験（`testing/CHANGELOG.md` 参照）で、Ollama呼び出しに `num_ctx` を明示指定していないと入力プロンプトだけで既定のコンテキスト長を超過し、Stage2の出力がほぼ空になりJSON解析に失敗する不具合を実際に確認した。`app/llm/client.py` / `app/config/settings.yaml`（`llm.num_ctx: 32768`）で明示指定するよう修正済み。
- **長時間の無音区間でのSTTハルシネーションに対応済み**：3時間耐久試験で `vad_enabled: false` のまま計測待ち等の無音区間が続くと、faster-whisperが同一フレーズを繰り返しハルシネーションし文字起こしの大半がゴミになる不具合を確認した。`vad_enabled: true` に変更して解消済み。スピーカー再生した音声をマイクで拾う検証方式では小さな音をVADが無音と誤判定するおそれがあるため、その場合は `vad_threshold` を下げる（[9. 音声処理パイプライン](#9-音声処理パイプライン)参照）。
- **依存ライブラリ経由の軽微な外部通信は解消済み**：`faster-whisper`が依存する`huggingface_hub`のエージェントハーネス検出機能により、起動時に`huggingface.co`への通信が発生していた。`docker-compose.yml`の`minutes-app`環境変数に`HF_HUB_OFFLINE=1`を追加して無効化済み。

---

## 18. ライセンス・クレジット

本番のLRシステム（`app/`配下）自体は音声合成を行わないためVOICEVOXを使用しないが、
`testing/`配下の立会試験シミュレーション（台本の読み上げによる検証用音声の生成）では
[VOICEVOX](https://voicevox.hiroshiba.jp/)を利用している。VOICEVOXソフトウェア利用規約
により、生成音声を利用する際はクレジット表記および各音声ライブラリ（キャラクター）ごとの
利用規約の遵守が必要。使用キャラクター・クレジット表記・利用規約URL・利用条件（企業利用時
の事前確認要否等）の詳細は [`testing/CREDITS.md`](testing/CREDITS.md) を参照。
